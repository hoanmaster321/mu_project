...
bool CGMRenderGroupMesh::runtime_render_obj(int Index, BMD* pModel, OBJECT* pObject, int Type)
{
	...
	if (Type == RENDER_MESH_OBJ_ITEM)
	{
		...
		if (data)
		{
			...
			{
				this->RenderSet(&data->Group[i], pModel, pObject);
	...
	if (renderaction == TRUE && (&edit_mesh)->Index == itemindex)
	{
		...
		if (edit_item.size() != 0)
		{
			for (size_t i = 0; i < edit_item.size(); i++)
			{
				...
				this->RenderSet(&edit_item[i], pModel, pObject);
			}

			this->RenderSet(&edit_mesh, pModel, pObject);
		}
		else
		{
	...
	else
	{
		...
		else
		{
			...
			if (Effectindex != -1)
			{
				is_rendered = true;
				VectorCopy(pModel->BodyLight, BodyLight);
				pModel->RenderSet(Effectindex, pObject);
				VectorCopy(BodyLight, pModel->BodyLight);
			}
		}
	}
...
bool CGMRenderGroupMesh::RenderSetEffect(int modelType, BMD* pModel, OBJECT* pObj, float Alpha, int RenderType, int Texture)
{
	bool is_rendered = false;
	...
	else
	{
		...
		else
		{
			...
			if (Effectindex != -1)
			{
				...
				pModel->RenderSetEffect(Effectindex, pObj, Alpha, RenderType, Texture);
				pModel->EndRender();
			}
		}
	}
...
void CGMRenderGroupMesh::RenderSet(RENDER_MESH_EFFECT* b, BMD* pModel, OBJECT* pObject)
{
	int Texture = 0;
	int blend_mesh = 0;
	int RenderFlag = 0;
...
bool CGMRenderGroupMesh::runtime_update_interface()
{
	...
	if (SEASON3B::CheckMouseIn(RenderFrameX, RenderFrameY, RenderSizeX, RenderSizeY))
	{
		...
		const int modelIdx = (&edit_mesh)->Index + MODEL_ITEM;
		BMD* pModel = (modelIdx >= 0 && modelIdx < MAX_MODELS && Models) ? &Models[modelIdx] : nullptr;
...
void CGMRenderGroupMesh::runtime_make_text(float RenderFrameX, float RenderFrameY)
{
	...
	const int modelIdx2 = (&edit_mesh)->Index + MODEL_ITEM;
	BMD* pModel = (modelIdx2 >= 0 && modelIdx2 < MAX_MODELS && Models) ? &Models[modelIdx2] : nullptr;
