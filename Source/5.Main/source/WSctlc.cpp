...
{	
	g_ConsoleDebug->Write(MCD_ERROR, "[Send Packet ] EncryptData 1");
	int nResult;
	...
	while( 1 ) 
	{
		...
		if (gProtect.CheckSocketPort(socket) != 0)
		{
			...
			g_ConsoleDebug->Write(MCD_ERROR, "[Send Packet ] EncryptData CWsctlc::sSend");
		}
#endif
...
{
	//g_ConsoleDebug->Write(MCD_ERROR, "[Send Packet ] EncryptData 2");
	int nResult;
	int nDx	  = 0;
	...
	while( m_nSendBufLen > 0 ) 
	{
		...
		{
			gProtect.EncryptData((BYTE*)m_SendBuf + nDx, m_nSendBufLen - nDx);
			g_ConsoleDebug->Write(MCD_ERROR, "[Send Packet ] EncryptData FDWriteSend()");
		}
#endif
...
{	
	...
	if (gProtect.CheckSocketPort(m_socket) != 0)
	{
		gProtect.DecryptData((BYTE*)m_RecvBuf + m_nRecvBufLen, MAX_RECVBUF - m_nRecvBufLen);
	}
#endif
	if( nResult == 0 )
	{
